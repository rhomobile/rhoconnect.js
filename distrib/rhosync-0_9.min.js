...
a))})};this.warning=function(a,b){var e=g.warning;l>e||d(function(d){d.warn(p(e,a));b&&d.warn("EXCEPTION: "+b)})};this.error=function(a,b){var e=g.error;l>e||d(function(d){d.error(p(e,a));b&&d.error("EXCEPTION: "+b)})};this.fatal=function(a,b){var e=g.fatal;l>e||d(function(d){d.error(p(e,a));b&&d.error("EXCEPTION: "+b)})}}var n=RhoSync.rho;a.extend(n,{Logger:k,ERRORS:RhoSync.ERRORS,deferredMapOn:function(b){var d={},p=[];a.each(b,function(b){var e=new a.Deferred;d[b]=e;p.push(e.promise())});return{resolve:function(a,
b){d[a]&&d[a].resolve.apply(d[a],b)},reject:function(a,b){d[a]&&d[a].reject.apply(d[a],b)},when:function(){return a.when.apply(this,p)}}},passRejectTo:function(a){return function(){a.reject(arguments)}}});a.extend(RhoSync,{Logger:k})})(jQuery);
...
this.clearNotifications=function(){};this.destroy=function(){};this.updateAttributes=function(){this.save()};this.save=function(){p=d=!1}}var n=RhoSync.rho;a.extend(n,{domain:{SyncObject:k,Model:function(a){this.source=new n.engine.Source(a.sourceId,a.name,"incremental",n.storage,n.engine,this);this.__defineGetter__("name",function(){return this.source.name});this.__defineSetter__("name",function(a){this.source.name=a});this.belongsTo=a.belongsTo;this.deleteAll=function(){};this.find=function(){};
...
1:0,c.refreshTime,c.id])}c.getNotify().reportSyncStatus("syncronizing"+c.name+"...",c.errCode,c.error);var e=Date.now();if(c.isTokenFromDb&&c.token>1)c.syncServerChanges().done(function(){d();a.resolve()}).fail(b);else{c.isEmptyToken()?c.processToken(1).done(function(){f()}).fail(b):f();var f=function(){c.syncClientChanges().done(function(e){e||c.syncServerChanges().done(function(){d();a.resolve()}).fail(b)}).fail(b)}}}).promise()};this.getNotify=function(){return this.engine.getNotify()};this.getInsertedCount=
function(){return this.insertedCount};this.getDeletedCount=function(){return this.deletedCount}}function q(a){return function(c,b){a.reject(i.ERRORS.ERR_RUNTIME,"db access error: "+b)}}function v(a){return function(c,b){a.reject(c,b)}}function K(a){this.id=a;this.token=this.session=null;this.reset=this.token_sent=0;this.last_sync_success=this.port=null}var i=RhoSync.rho,w={none:0,syncAllSources:1,syncSource:2,search:3,stop:4,exit:5},D={},x=[],V=new function(){var a={};this.setProperty=function(c,
...
a);a==-1?C=null:u[a]=null};this.cleanLastSyncObjectCount=function(){o={}};this.incLastSyncObjectCount=function(a){var b=o[a]||0;b+=1;return(o[a]=b)||0};this.callLoginCallback=function(a,b,f){d.isStoppedByUser()||(b="error_code="+b,b+="&error_message="+(f!=null?f:""),b+="&rho_callback=1",l.info("Login callback: "+a.toString()+". Body: "+b),e(a,b))}},SyncNotification:k,byEvent:function(b){a(window).trigger(jQuery.Event(b),a.makeArray(arguments).slice(1))}}});a.extend(RhoSync,{SyncNotification:k})})(jQuery);
Ext&&function(a,k){var n=null;k.data.RhosyncStorageProxy=k.extend(k.data.ClientProxy,{LOG:new RhoSync.rho.Logger("Rhosync.plugin-extjs.js"),dbName:void 0,constructor:function(a){k.data.RhosyncStorageProxy.superclass.constructor.call(this,a);this.setReader(this.reader);if(this.getStorageObject()==void 0)throw"Rhosync Storage is not available, please ensure you have rhosync.js scripts properly loaded";this.dbName=this.dbName||(this.store?this.store.storeId:void 0);if(this.dbName==void 0)throw"No database name was provided to the rhosync storage proxy. See Ext.data.RhosyncStorageProxy documentation for details";
this.initialize()},create:function(b,d,p){function g(){b.setCompleted();typeof d=="function"&&d.call(p||this,b)}var e=this,l=b.records,j;b.setStarted();var k=RhoSync.rho.deferredMapOn(l);a.each(l,function(a,b){b.phantom?(b.phantom=!1,j=e.getNextId()):j=b.getId();e.setRecord(b,j).done(function(b){k.resolve(a,[b])}).fail(function(b,d){k.reject(a,[b,d])})});k.when().done(function(){b.setSuccessful();g()}).fail(function(){g();e.LOG.error("update() object update error")})},read:function(a,d,p){function g(){var g=
null;e.root?(g={},g.name=l,g[e.root]=s,g=j.read(g)):g=j.read(s);k.apply(a,{resultSet:g});a.setCompleted();typeof d=="function"&&d.call(p||e,a)}var e=this,l=e.model.modelName,j=e.getReader(),s=[];a.id?e.getRecord(a.id).done(function(d){d&&(s.push(d),a.setSuccessful());g()}).fail(function(a,b){e.LOG.error("read() single object read error: "+b);g()}):e.findAll(l).done(function(d){s=d;a.setSuccessful();g()}).fail(function(a,b){e.LOG.error("read() all objects read error: "+b);g()})},update:function(b,
d,k){function g(){b.setCompleted();typeof d=="function"&&d.call(k||this,b)}var e=this,l=b.records;b.setStarted();var j=RhoSync.rho.deferredMapOn(l);a.each(l,function(a,b){e.setRecord(b).done(function(b){j.resolve(a,[b])}).fail(function(b,d){j.reject(a,[b,d])})});j.when().done(function(){b.setSuccessful();g()}).fail(function(){g();e.LOG.error("update() object update error")})},destroy:function(b,d,k){function g(){b.setCompleted();typeof d=="function"&&d.call(k||this,b)}var e=this,l=b.records;b.setStarted();
var j=RhoSync.rho.deferredMapOn(l);a.each(l,function(a,b){e.removeRecord(b).done(function(b){j.resolve(a,[b])}).fail(function(b,d){j.reject(a,[b,d])})});j.when().done(function(){b.setSuccessful();g()}).fail(function(){g();e.LOG.error("update() object update error")})},findAll:function(b){function d(a,b){var d={},g=k.model.prototype.fields.items,h=g.length,m,f,n;for(m=0;m<h;m++)f=g[m],n=f.name,d[n]=typeof f.decode=="function"?f.decode(b[n]):b[n];d.id=a;return d}var k=this,g=k.getStorageObject();return a.Deferred(function(e){function l(a){g.executeSql("SELECT * FROM object_values WHERE source_id=?",
[a]).done(function(a,b){for(var d={},e=0;e<b.rows.length;e++){var g=b.rows.item(e).object,k=b.rows.item(e).attrib,l=b.rows.item(e).value,c=d;c[g]||(c[g]={});c[g][k]=l}j(d)}).fail(function(a,b){k.LOG.error("findAll() select all objects for source error: "+b);e.reject(a,b)})}function j(b){var g=[];a.each(b,function(a,b){g.push(d(a,b))});e.resolve(g)}g.executeSql("SELECT source_id FROM sources WHERE name=? LIMIT 1 OFFSET 0",[b]).done(function(a,b){var d=null;b.rows&&b.rows.length&&b.rows.length>0&&(d=
b.rows.item(0).source_id);l(d)}).fail(function(a,b){k.LOG.error("findAll() source_id select error: "+b);e.reject(a,b)})}).promise()},getRecord:function(b){function d(a){var d={},g=k.model.prototype.fields.items,n=g.length,h,m,f={};for(h=0;h<a.rows.length;h++)m=a.rows.item(h).attrib,f[m]=a.rows.item(h).value;for(h=0;h<n;h++)a=g[h],m=a.name,d[m]=typeof a.decode=="function"?a.decode(f[m]):f[m];d.id=b;return d}var k=this,g=k.getStorageObject();return a.Deferred(function(a){g.executeSql("SELECT * FROM object_values WHERE object=?",
[b.toString()]).done(function(b,g){a.resolve(d(g))}).fail(function(b,d){k.LOG.error("getRecord() error: "+d);a.reject(b,d)})}).promise()},setRecord:function(b,d){var k=this,g=k.getStorageObject(),e=k.model.modelName,l=null,j=!1;d?(b.setId(d),j=!0):d=b.getId();var n=b.data,h={},m=k.model.prototype.fields.items;return a.Deferred(function(f){function u(){g.executeSql("SELECT attrib FROM object_values WHERE object=?",[d.toString()]).done(function(a,b){for(var d=0;d<b.rows.length;d++){var e=b.rows.item(d).attrib;
e&&(t[e]=!0)}C()}).fail(function(a,b){k.LOG.error("setRecord() read attr names error: "+b);f.reject(a,b)})}function C(){g.rwTx().ready(function(e,c){a.each(m,function(a,e){var f=e.name;h[f]=typeof e.encode=="function"?e.encode(n[f],b):n[f];var k=t[f]?"UPDATE object_values SET value=? WHERE source_id=? and object=? and attrib=?":"INSERT INTO object_values ( value, source_id, object, attrib ) VALUES (?, ?, ?, ?)",m=h[f];f!="id"&&(g.executeSql(k,[m,l.toString(),d.toString(),f],c),j||g.executeSql("INSERT INTO changed_values ( value, source_id, object, attrib, update_type ) VALUES (?, ?, ?, ?, ?)",
[m,l.toString(),d.toString(),f,"update"],c))});j&&g.executeSql("INSERT INTO changed_values ( value, source_id, object, attrib, update_type ) VALUES (?, ?, ?, ?, ?)",[null,l.toString(),d.toString(),"object","create"],c)}).done(function(){b.dirty=!1;f.resolve(d)}).fail(function(){f.reject(null,"setRecord() update/insert attr error")})}if(void 0!=b.dirty&&!b.dirty)f.resolve(d);else{g.executeSql("SELECT source_id FROM sources WHERE name=?",[e]).done(function(a,b){b.rows&&b.rows.length&&b.rows.length>
0&&(l=b.rows.item(0).source_id);u()}).fail(function(a,b){k.LOG.error("setRecord() read source_id error: "+b);f.reject(a,b)});var t={}}}).promise()},removeRecord:function(b){var d=this,k=d.getStorageObject(),g=d.model.modelName,e=null,l=null;if(b&&b.data&&b.data.id)l=b.data.id;return a.Deferred(function(j){function n(){k.rwTx().ready(function(b,d){function f(){function b(){a.each(g,function(a,b){k.executeSql("INSERT INTO changed_values ( value, source_id, object, attrib, update_type ) VALUES (?, ?, ?, ?, ?)",
[b,e.toString(),l.toString(),a,f],d)})}var f="delete";k.executeSql("SELECT update_type FROM changed_values WHERE object=? AND update_type=? AND sent=?",[l.toString(),"create",0],d).done(function(d,c){0<c.rows.length&&(f=null);k.executeSql("DELETE FROM changed_values WHERE object=? AND source_id=? AND sent=?",[l.toString(),e.toString(),0],d).done(function(){var c=!1;a.each(g,function(){f&&(c=!0)});c&&b()})})}var g={};k.executeSql("SELECT * FROM object_values WHERE object=? AND source_id=?",[l.toString(),
e.toString()],d).done(function(a,b){for(var d=0;d<b.rows.length;d++){var c=b.rows.item(d).attrib,h=b.rows.item(d).value;c&&(g[c]=h)}k.executeSql("DELETE FROM object_values WHERE object=? AND source_id=?",[l.toString(),e.toString()],a).done(function(){f()})})}).done(function(){b.dirty=!1;j.resolve(l)}).fail(function(){j.reject(null,"setRecord() remove attr error")})}k.executeSql("SELECT source_id FROM sources WHERE name=?",[g]).done(function(a,b){b.rows&&b.rows.length&&b.rows.length>0&&(e=b.rows.item(0).source_id);
n()}).fail(function(a,b){d.LOG.error("setRecord() read source_id error: "+b);j.reject(a,b)})}).promise()},initialize:function(){},clear:k.emptyFn,getStorageObject:function(){return RhoSync.rho.storage},getNextId:function(){n=n||Date.now()-(new Date(2009,1,1)).getTime();n+=1;return n}});k.data.ProxyMgr.registerType("rhosync",k.data.RhosyncStorageProxy)}(jQuery,Ext);
